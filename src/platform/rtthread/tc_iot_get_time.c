...
static uint32_t _get_timestamp_from_ntp(const char *host)
{
    long fd;  /* socket */
    int ret = -1;
    struct timeval_t tv;
    ...
    fd = (long)tc_iot_hal_udp_create((char *)host, NTP_PORT);
    if (fd < 0) {
        TC_IOT_LOG_ERROR("udp create error!");
        return 0;
    }
    ...
    ret = tc_iot_hal_udp_write((void *)fd, write_buf, write_len);
    if (ret < 0) {
        TC_IOT_LOG_ERROR("udp write error!");
        goto do_exit;
    }

    ret = tc_iot_hal_udp_readTimeout((void *)fd, read_buf, sizeof(read_buf), 3000);
    if (ret < 0) {
